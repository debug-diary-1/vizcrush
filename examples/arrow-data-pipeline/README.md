...
It demonstrates format interoperability without claiming a zero-copy decode. The UI names and measures Arrow decode separately from the concurrent vizcrush preprocessing stage and reports the backend that each kernel actually executed. Both kernels make one untimed call on the decoded real input before that stage is measured, excluding lazy WASM loading. The payload is generated locally to keep the example self-contained.
