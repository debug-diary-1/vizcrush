...
`engines.node` requires Node 24. Install Node 24 (`mise install` in the repo root, or `nvm install 24`). The warning is non-fatal but several dependencies assume Node 24+.
